...
(() => {
  ...
  function normalizeBoard(data) {
    ...
    return uniqueBestByName(list.map(normalizeEntry).filter(Boolean));
  }
  ...
  async function submitScore(playerName, score) {
    ...
    const name = String(playerName || '').trim().toUpperCase().slice(0, 12);
    const fromBoard = board.findIndex((e) => e.name === name);
    const rank = Number(data?.rank);
    return {
      board,
      rank: fromBoard >= 0 ? fromBoard + 1 : (Number.isFinite(rank) ? rank : -1),
    };
  }
